...
def main(argv):
	...
	if len(argv) != 1:
		sys.exit('Usage: user_pass.py <file_name or username> ')

	print '\nUser & Password Authentication Program v.01\n'

	user_names, passwords = process_file(sys.argv[0])
	...
	#while pass_try < x:
		user_input = hashlib.sha224(getpass.getpass(passwords)).hexdigest()
